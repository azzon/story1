...
TODO_FILE = ROOT / "novel-creation.todos.md"  # 新的单文件TODO系统
...
**这是一个小说创作任务，请按照TODO中的具体要求完成。**

## 📋 核心要求

**这是一个小说创作任务，请直接按照TODO中的要求完成创作。**

### 执行步骤：

1. **阅读任务**：仔细阅读TODO中的所有要求（字数、场景、情绪、钩子设计等）

2. **完成创作**：
   - 严格按照字数要求撰写（±10%可接受）
   - 确保开篇无缝衔接上一章（禁止"第二天/与此同时"等归零开头）
   - 用"五感描写"呈现场景（看到/听到/闻到/感觉到）
   - 对话要符合人物性格（不同角色说话方式不同）
   - 章末必须有钩子（动作进行中/真相揭示一半/危机降临/两难抉择）
...
4. **输出文件**：按TODO指定的路径保存文件

### ⚠️ 语言质量要求

**避免AI痕迹词**：
- 然而/显然/不得不说 → 改为更自然的表达
- 嘴角勾起/眼中闪过一丝 → 改为具体的动作描写
- 就在这时/突然 → 减少使用，改为环境渲染
...
- [ ] 按要求完成创作（字数/场景/情绪达标）
- [ ] 文件已保存到指定路径
- [ ] TODO已勾选（修改 `{stage_file}`，将 `- [ ]` 改为 `- [x]`）
- [ ] 通过3个终极问题验收
- [ ] PR描述包含 `Fixes #{{issue_number}}`
...
def iter_work_items(todo_file: Path, batch_size: int, completed_ids: set[str]) -> List[WorkItem]:
    ...
    Args:
        todo_file: TODO文件路径（novel-creation.todos.md）
        batch_size: 每个Issue包含的TODO数量
        completed_ids: 已完成的TODO ID集合

    Returns:
        当前需要处理的工作项列表
    """
    if not todo_file.exists():
        logger.warning(f"TODO 文件不存在: {todo_file}")
        return []
...
class Pipeline:
    ...
    def _build_full_issue_body(self, item: WorkItem, task_details: str) -> str:
        ...
        reference_files = f"""- **完整TODO列表**: `novel-creation.todos.md`（包含所有创作步骤）
- **当前任务文件**: `{relative_path}`
- **世界观设定**: `world-bible.md`（如不存在将在任务中创建）
- **人物档案**: `characters.md`（如不存在将在任务中创建）
- **故事大纲**: `outline.md`（如不存在将在任务中创建）
- **章节文件**: `chapters/vol-XX/chapter-XXXX.md`（按TODO指定路径创建）"""
    ...
    try:
        ...
        while True:
            try:
                ...
                work_items = iter_work_items(TODO_FILE, args.issue_batch_size, completed_ids)
